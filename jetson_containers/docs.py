...
from jetson_containers import (
    ...
    find_package,
    find_packages,
    find_registry_containers,
    group_packages,
    package_scan_options,
...
def generate_package_docs(packages, root, repo, simulate=False):
    ...
    for pkg_path, pkgs in groups.items():
        ...
        run_txt += "To mount your own directories into the container, use the [`-v`](https://docs.docker.com/engine/reference/commandline/run/#volume) or [`--volume`](https://docs.docker.com/engine/reference/commandline/run/#volume) flags:\n"
        run_txt += "```bash\n"
        run_txt += f"jetson-containers run -v /path/on/host:/path/in/container $(autotag {pkg_name})\n"
        run_txt += "```\n"
        run_txt += "To launch the container running a command, as opposed to an interactive shell:\n"
        run_txt += "```bash\n"
