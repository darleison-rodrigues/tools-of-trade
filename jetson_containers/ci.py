...
def register_runner(token, root, repo, labels=[], simulate=False):
    ...
    """
    if not args.token:
        raise ValueError("--token must be provided from GitHub when registering self-hosted runners")
    ...
    print(f"  cd {root}\n")
    print("  sudo ./svc.sh start      # manually start the service")
    print("  sudo ./svc.sh stop       # manually stop the service")
    print("  sudo ./svc.sh status     # check the service status")
    print("  sudo ./svc.sh uninstall  # remove systemd service\n")
