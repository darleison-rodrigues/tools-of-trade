...
from .utils import get_env_flag # Assuming get_repo_dir is not needed here
...
LogConfig = types.SimpleNamespace(
    ...
    dir = os.path.join(
        os.getcwd(), 'logs', # Changed get_repo_dir() to os.getcwd()
...
def colorize(text, color=None, on_color=None, attrs=[]):
    ...
    text = text.replace('<b>', '\033[1m').replace('</b>', '\033[22m')
...
def log_versions(keys=None, columns=4, color='green', tablefmt='simple_grid'):
    ...
    """
    from .l4t_version import L4T_VERSION, JETPACK_VERSION, CUDA_VERSION, PYTHON_VERSION, SYSTEM_ARCH, LSB_RELEASE # Direct import
    ...
    for key in keys:
        # Use getattr with a default to avoid errors if a variable is not defined
        value = getattr(sys.modules[__name__], key, 'N/A') # Access directly from module
        text = [key, f"{value}"]
...
def log_table( rows, header=None, footer=None,
    ...
    if wrap_rows and len(rows) > wrap_rows:
        for i in range(wrap_rows, len(rows)):            rows[i % wrap_rows].extend(rows[i])        rows = rows[:wrap_rows]
